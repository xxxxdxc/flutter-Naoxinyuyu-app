...
class _DevicePanelContentState extends State<_DevicePanelContent> {
  ...
  String _getHrvSubtitle(GlobalAppState state) {
    ...
    if (state.isDemoMode) {
      if (state.hrvConnection.status == ConnectionStatus.connected) {
        return '${state.hrvConnection.deviceName} 已连接 (电量 ${state.batteryPct}%)';
      }
      return '${state.hrvConnection.deviceName ?? '演示设备 - HRV 胸带'} 待连接';
    }
  ...
  String _getDbsSubtitle(GlobalAppState state) {
    ...
    if (state.isDbsConnected) {
      ...
      if (state.isDemoMode) {
        return '${state.dbsConnection.deviceName ?? '演示设备 - DBS'} 已连接 (电量 ${state.dbsBatteryPercent}%$temp)';
      }
  ...
  Widget _buildHrvTrailing(GlobalAppState state) {
    ...
    if (state.isDemoMode) {
      return const Chip(label: Text('演示'));
    }
  ...
  Widget _buildDemoToggle(GlobalAppState state) {
    final active = state.isDemoMode;
    return SizedBox(
      width: double.infinity,
      child: ElevatedButton.icon(
        onPressed: () async {
          if (active) {
            await state.stopHardwareDemo();
          } else {
            await state.startHardwareDemo();
          }
          if (!mounted) return;
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text(active ? '已停止演示模式' : '演示模式已启动')),
          );
        },
        icon: Icon(active ? Icons.stop_circle_outlined : Icons.play_circle),
        label: Text(active ? '停止演示模式' : '启动演示模式'),
        style: ElevatedButton.styleFrom(
          backgroundColor: active ? AppTheme.error : AppTheme.brandPurple,
          foregroundColor: Colors.white,
          ...
          children: [
            Expanded(
              child: _DeviceCard(
                ...
                subtitle: state.hrvConnection.deviceName,
              ),
            ),
            ...
            Expanded(
              child: _DeviceCard(
                ...
                subtitle: state.dbsConnection.deviceName,
              ),
            ),
          ],
        );
...
class _SevenStepProgress extends StatelessWidget {
  ...
  const _SevenStepProgress({
    required this.done,
    required this.total,
    this.activeColor = AppTheme.brandPurple,
  });

  @override
  Widget build(BuildContext context) {
    final safeTotal = total <= 0 ? 7 : total;
    final safeDone = done.clamp(0, safeTotal);
    return Row(
      children: [
...
class _CalibrationPanel extends StatelessWidget {
      ...
      builder: (context, state, child) {
        ...
        switch (state.calPhase) {
          ...
          case CalibrationPhase.enteringHrvCalibration:
            return _buildEnteringHrvCalibration(context, state);
          case CalibrationPhase.connectingChestStrap:
            return _buildConnectingChestStrap(context, state);
          case CalibrationPhase.checkingSignalQuality:
            return _buildSignalQualityCheck(context, state);
          ...
          case CalibrationPhase.generatingModel:
            return _buildModelStep(
              context,
              title: '生成个体化模型',
              message: '正在基于静息基线和应激诱导数据生成个体化 HRV 模型…',
  ...
  Widget _buildEnteringHrvCalibration(
    BuildContext context,
    GlobalAppState state,
  ) {
    return _buildCard(
      ...
      title: Row(
        children: [
          const Icon(Icons.assignment_outlined, color: AppTheme.brandPurple),
          const SizedBox(width: 8),
          Text(
            '进入 HRV 校准',
      ...
      body: Column(
        ...
        children: [
          Text(
            '流程：连接胸带 → 信号质量检查 → 静息基线 7×60s → 应激诱导 2×60s → 生成并保存模型。',
            style: Theme.of(
              context,
            ).textTheme.bodySmall?.copyWith(color: AppTheme.textSecondary),
          ),
          ...
          SizedBox(
            width: double.infinity,
            child: ElevatedButton.icon(
              onPressed: state.isDemoMode
                  ? () => state.connectDemoChestStrap()
                  : null,
              icon: const Icon(Icons.bluetooth_connected),
              label: const Text('连接演示设备 - HRV 胸带'),
              style: ElevatedButton.styleFrom(
  ...
  Widget _buildConnectingChestStrap(
    BuildContext context,
    GlobalAppState state,
  ) {
    return _buildCard(
      context: context,
      accentColor: AppTheme.deviceHrv,
      title: Row(
        children: [
          const Icon(Icons.bluetooth_connected, color: AppTheme.deviceHrv),
          const SizedBox(width: 8),
          Text(
            '连接胸带',
            style: Theme.of(context).textTheme.titleSmall?.copyWith(
      ...
      body: Column(
        ...
        children: [
          Text(
            '${state.hrvConnection.deviceName ?? '演示设备 - HRV 胸带'} 已连接，准备进行信号质量检查。',
            style: Theme.of(
              context,
            ).textTheme.bodySmall?.copyWith(color: AppTheme.textSecondary),
          ),
          ...
          SizedBox(
            width: double.infinity,
            child: ElevatedButton.icon(
              onPressed: () => state.startDemoSignalQualityCheck(),
              icon: const Icon(Icons.fact_check_outlined),
              label: const Text('开始信号质量检查'),
            ),
          ),
        ],
      ),
  ...
  Widget _buildSignalQualityCheck(BuildContext context, GlobalAppState state) {
    ...
    return _buildCard(
      ...
      body: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          LinearProgressIndicator(value: quality / 100),
          const SizedBox(height: 10),
          Text(
            quality >= 90 ? '信号质量良好，系统将自动进入静息基线采集。' : '正在稳定信号，请保持胸带贴合。',
            style: Theme.of(
              context,
            ).textTheme.bodySmall?.copyWith(color: AppTheme.textSecondary),
          ),
  ...
  Widget _buildModelStep(
    ...
    return _buildCard(
      ...
      title: Row(
        children: [
          Icon(icon, color: AppTheme.brandPurple),
          const SizedBox(width: 8),
          Text(
            title,
            style: Theme.of(context).textTheme.titleSmall?.copyWith(
    ...
    return _buildCard(
      ...
      body: Column(
        ...
        children: [
          ...
          const SizedBox(height: 10),
          _SevenStepProgress(done: state.calmDone, total: state.calmNeed),
          const SizedBox(height: 12),
          SizedBox(
            width: double.infinity,
            child: ElevatedButton.icon(
  ...
  Widget _buildCalibrating(BuildContext context, GlobalAppState state) {
    ...
    final remaining = state.currentSegmentRemainingSeconds;
      ...
      body: Column(
        ...
        children: [
          ...
          _SevenStepProgress(
            ...
            total: state.calmNeed,
            activeColor: Colors.orangeAccent,
          ),
          const SizedBox(height: 12),
          Text(
            remaining > 0
                ? '静息基线采集：每段 60 秒，当前段剩余 $remaining 秒。请保持安静放松状态。'
    ...
    return _buildCard(
      ...
      body: Column(
        ...
        children: [
          ...
          const SizedBox(height: 10),
          _SevenStepProgress(done: state.calmNeed, total: state.calmNeed),
          const SizedBox(height: 4),
          Text(
